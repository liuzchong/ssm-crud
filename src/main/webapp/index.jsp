...
<%--引入标签库c cforeach把数据便利出来 cif进行判断--%>
...
<script type="text/javascript">
    ...
    function to_page(pn) {
        $.ajax({
            ...
            success: function (result) {
                //console.log(result); 调用函数处理信息，这里的result即ajax请求获得的结果
                //1、解析并显示员工数据
    ...
    function build_emps_table(result) {
        ...
        $.each(emps, function (index, item) {
            ...
            //为编辑按钮添加一个自定义的属性，来表示当前员工id(编辑员工信息时候使用)
    ...
    function build_page_nav(result) {
        ...
        //遍历的是页码号：1,2,3,4,5(pageInfo里有navigatepageNums[]，所以直接遍历它就行了)
        //遍历给ul中添加页码提示
    ...
    $("#emp_add_modal_btn").click(function () {
        //清除表单数据（表单重置）  jQuery没有reset方法，使用的是dom对象的reset()方法
        //也要清空样式
        reset_form("#empAddModal form");
        //发出ajax请求，查出部门信息，并显示在下拉列表中
    ...
    function validate_add_form() {
        ...
        if (!regName.test(empName)) {
            show_validate_msg("#empName_add_input", "error", "用户名必须是2-5位中文或者6-16位英文");
            return false;
        } else {
    ...
    function getEmp(id) {
        $.ajax({
            ...
            success: function (result) {
                ...
                //将员工属性的各个信息添加到对话框的指定位置即可
                $("#empName_update_static").text(empData.empName);
                //注意val和value
                $("#email_update_input").val(empData.email);
                //通过name属性值去查找元素，注意给复选框赋值的方式
    ...
    $("#emp_update_btn").click(function () {
        ...
        $.ajax({
            ...
            type: "POST",
            data: $("#empUpdateModal form").serialize() + "&_method=PUT",//表单中的数据序列化
            success: function (result) {
                //alert(result.msg);
                //1、关闭对话框
    ...
    $("#check_all").click(function () {
        ...
        //prop可以修改和获取dom原生属性的值，attr获取自定义属性的值
        //从.check_item看prop方法是给他赋值，从this看prop方法是取值
        $(".check_item").prop("checked",$(this).prop("checked"));
    });

    //check_item(因为这个框也是后来创建的，所以需要用on)
